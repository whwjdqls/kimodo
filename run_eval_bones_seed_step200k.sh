#!/bin/bash
# Evaluate a single bones_seed checkpoint (default: ckpt_step0200000.pt) on the
# 20fps text2motion benchmark. Pipeline (matches run_eval_bones_seed_text2motion.sh):
#   build_eval_model_folder (--fps 20) -> generate_eval -> embed_folder (20->30)
#   -> evaluate_folder (--fps 20) -> parse_folder (summary_rows.json + md table)
# Artifacts land under the model's run dir: runs/bones_seed/eval/{models,gen,logs}/<label>.
# Uses the precomputed LLM2Vec text cache (no live 8B encoder). Set TEXT_CACHE="" to
# fall back to the live encoder.
#
# Usage:
#   sbatch run_eval_bones_seed_step200k.sh                      # default ckpt_step0200000.pt
#   sbatch run_eval_bones_seed_step200k.sh ckpt_step0150000.pt  # any bones_seed checkpoint
#SBATCH --job-name=eval_bseed_200k
#SBATCH --partition=a2
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=/home/jungbin_cho/kimodo_open/runs/bones_seed/eval/logs/%x_%j.log
#SBATCH --error=/home/jungbin_cho/kimodo_open/runs/bones_seed/eval/logs/%x_%j.err
...
CKPT="${1:-ckpt_step0200000.pt}"     # override as positional arg
...
# label: ckpt_step0200000.pt -> step_200000  (fallback: ckpt stem)
num=$(echo "${CKPT}" | grep -oP '[0-9]+' || true)
