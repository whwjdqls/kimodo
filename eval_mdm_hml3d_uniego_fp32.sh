#!/bin/bash
# Evaluate runs/mdm_hml3d_uniego_fp32 with the MoMask/T2M evaluator.
#
# MDM one-stage on the UniEgoMotion-style 211-D head-centric representation.
# Uses the uniego dispatch in evaluation/eval_hml3d.py (211 -> uniego_to_kimodo
# -> kimodo_to_humanml3d -> HML3D 263 for the evaluator; joints decoded via
# uniego_world_joints_from_features for the foot metrics).
#
# Outputs:
#   runs/mdm_hml3d_uniego_fp32/eval_<ckpt-stem>.json
#   runs/mdm_hml3d_uniego_fp32/eval_<ckpt-stem>.log

set -uo pipefail

# --------- knobs (env overridable) ---------
RUN_NAME="mdm_hml3d_uniego_fp32"
CKPT_NAME="${CKPT_NAME:-ckpt_step0450000.pt}"
